...
public class ThreadPoolManager {
	...
	private ArrayList<WorkerThreads> workerList; // used to get statuses of the
													// threads in cp

	public ThreadPoolManager(int capacity, ServerSocket serverSocket, Container container, String ROOT) {
	...
	private void initAllWorkers() throws Exception // initializes and starts all
													// the workers
	{
		for (Integer i = 0; i < THREADPOOL_CAPACITY; i++) {
			WorkerThreads worker = new WorkerThreads(requestQueue, "Thread_" + i.toString(), this, container, ROOT);
	...
	public synchronized void shutdown() {
		...
		System.out.println("Server is shutting down\nThreads are put to death"); // print
																					// to
																					// console
		...
		// !IMPORTANT: a seperate flag for interrupt handling is used here
		// instead of the in built one,
		// this is because once the the Thread.interrupted() is called, the flag
		// will be reset if it were true
		// and in my server the flag needs to be checked more than once
		...
		System.exit(0); // I used this since the main thread doesn't exit for
						// some reason in eclipse
	}
